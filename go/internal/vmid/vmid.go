// Package vmid derives everything predictable about a VM from its id NNN:
...
// platform a VM runs on is an explicit --backend value (see
...
// VM's internal 10.163.<NNN>.0/24 subnet, the last octet of its LAN address
...
// special ids — sandVM VMs use ordinary ids from the same range.
//
...
// ID is a validated VM id, 100-254.
type ID int

// Parse validates an id string and returns it. An id is 100-254: it doubles
// as the VM's subnet octet (255 is the broadcast address), and starting at
...
func Parse(s string) (ID, error) {
	...
	if n < 100 || n > 254 {
		return 0, fmt.Errorf("id %d out of range — a VM id is 100-254", n)
	}
	return ID(n), nil
}
...
// Name is the VM's hostname, mpd-<NNN>.
func (id ID) Name() string { return "mpd-" + id.String() }

// Zone is the VM's DNS zone, <NNN>.mpd.test.
