...
// would have to re-trust everywhere. And the dev's own env file (vm.env),
// because it is the dev's own writing rather than state
// mpd-virt generated — losing it to a verb that promises to be "fully
// recoverable" would be a lie, and it is not reproducible from anything
// left on the machine.
//
...
func uninstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "uninstall",
		Short: "Remove mpd-virt from this Mac: stop VMs (keep them) + wipe host state (keep the root CA + your vm.env)",
		Long: "Stops every adopted VM through its backend (container/parallels/utm;\n" +
			...
			"reuses it with no re-trust, and so is your own vm.env — this\n" +
			"tool never wrote it and cannot reproduce it. mpd-proxy and CA\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("uninstall mpd-virt — stops %d VM (kept, not deleted), then wipes ~/.mpd-virt (keeping the root CA and your vm.env) and the ssh-config blocks.\n", len(entries))
			...
			//    the dev's own vm.env.
			if err := wipeHostStateKeepingOwn(); err != nil {
				return err
			}
			pass("removed " + paths.Root() + " (kept the root CA and your vm.env)")
...
// root CA directory (conf/caroot) and the dev's own vm.env: all per-VM registry
// dirs and the rest of conf (servers, lan-hosts, service, backend.env).
// Keeping caroot is what lets a re-adopt reuse the same trust anchor;
// keeping the env files is because mpd-virt never wrote them and cannot
// reproduce them. A missing ~/.mpd-virt is a no-op.
//
