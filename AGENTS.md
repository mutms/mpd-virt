...
`notes` is the cached backend note `list` shows (proxmox only today) — a display
cache, sticky across lifecycle verbs: `adopt`/`start` save no note and leave it
untouched, only `list`'s refresh rewrites it.
...
└── <NNN>/                         ← per-VM, removed by `remove`
    ├── vm.json                    ← registry record + cached notes (see Registry above)
