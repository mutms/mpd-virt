#!/usr/bin/env bash
# Build the takeovertest image, boot it as a systemd box under `container run`,
...
container build -t takeovertest .
...
container run -d --name "$NAME" --cap-add ALL --memory "$MEMORY" takeovertest
