...
extension MpdVirt.CA {
    ...
    ///
    /// DNS names only, deliberately. The root constrains `dNSName` to
    /// `mpd.test`, but under RFC 5280 a name type absent from
    /// `permittedSubtrees` is unconstrained — so an `iPAddress` SAN would
    /// be the one field the constraint does not cover. Reach these hosts
    /// by name.
    static func issueLeaf(
        sans: [String], certPath: String, keyPath: String
    ) throws {
